...
@tf.function
def train_step_black_box(data, labels_one_hot, samples, weights = None, _lambda = 1., trainable = tf.constant(False)):
    cross_ent = tf.keras.losses.CategoricalCrossentropy()
    with tf.GradientTape() as tape:

        
        cross_entropy = cross_ent(labels_one_hot, black_box(data, trainable = trainable))
...
if __name__== "__main__":
    ...
    diff = []
    directory = f"images_black_box_NN/datasets_adapt_bis/{dataset_name}_{config_params['fold']}"
    ...
    for step in range(hyper_params["global_steps"]):
        ...
        for j in range(hyper_params["bb_steps"]):
            ...
            loss1 = train_step_black_box(data = tf.constant(X_train),
                                                      labels_one_hot = tf.constant(y_train_onehot), samples = None,
                                                      weights = None,
        ...
        y_train_bb = np.argmax(black_box.predict(X_train, trainable = False).numpy(), axis = 1)
        y_test_bb = np.argmax(black_box.predict(X_test, trainable  = False).numpy(), axis = 1)
