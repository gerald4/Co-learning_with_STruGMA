...
if __name__== "__main__":
    ...
    dataset_name = "read_gaussian_3_quantiles"
    ...
    directory = f"{directory}/boundaries_importance"
...
print('----------Training----------')
print(f"accuracy black box: {accuracy_score(y_train, np.argmax(black_box.predict(X_train).numpy(), axis = 1))}")
print(f"accuracy sTGMA: {accuracy_score(y_train, model.predict(X_train))}")
print(f"Fidelity: {accuracy_score(model.predict(X_train), np.argmax(black_box.predict(X_train).numpy(), axis = 1))}")

print('----------Testing----------')
print(f"accuracy black box: {accuracy_score(y_test, np.argmax(black_box.predict(X_test).numpy(), axis = 1))}")
print(f"accuracy sTGMA: {accuracy_score(y_test, model.predict(X_test))}")
print(f"Fidelity: {accuracy_score(model.predict(X_test), np.argmax(black_box.predict(X_test).numpy(), axis = 1))}")