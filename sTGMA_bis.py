...
class SoftTruncatedGaussianMixtureAnalysis(tf.Module):
    ...
    def gmm_initialisation(self, X_train, y_train):
        ...
        self.y_unique_t = tf.constant(y_unique)
    ...
    def normalizing_constant(self, way = "independent"):
        ...
        """
        print("---Tracing---Normalising constant")
        if way == "independent":
    ...
    def compute_log_pdf(self, X, y):
        print("---Tracning---compute_Log_pdf")
        dist = tfd.Mixture(
    ...
    def sample_per_class(self, nb_samples):
        ...
        for i in tf.range(len(self.y_unique)):
    ...
    def projection(self, X, y, resp, weights, t_range):
        ...
        print("-----Tracing---Projection")
        @tf.function
        def expec_ll(alpha1, alpha2):#, i, j, c1, c2):
            print("----Tracning---Expec_ll")
    ...
    def sample_importance(self, nb_samples):
        print("-----Tracing-sample_importance")
        list_samples = tf.TensorArray(dtype =tf.float32, size =0, dynamic_size= True)
        list_weights = tf.TensorArray(dtype =tf.float32, size =0, dynamic_size= True)
        tic = time()
        for i in tf.range(self.y_unique_t.shape[0]):
            ...
            list_samples = list_samples.write(i, samples)
            list_weights = list_weights.write(i,
        ...
        return tf.concat([list_samples.read(i) for i in range(self.y_unique.shape[0])], axis = 0) #, tf.concat(list_weights, axis = 0)
    ...
    def log_joint_prob(self, X):
        ...
        list_joint = tf.TensorArray(dtype =tf.float32,  size =self.y_unique_t.shape[0])
        for c in tf.range(self.y_unique_t.shape[0]):
            list_joint = list_joint.write(c, tf.reduce_logsumexp(self.compute_log_pdf(X, c), axis = -1) + tf.math.log(self.logits_y[c] + self.stable))
    ...
    @tf.function(input_signature=(tf.TensorSpec(shape=[None, None], dtype=tf.float32),))
    def log_pdf(self, X):
        print("----Tracing---log_pdf")
        return tf.reduce_logsumexp(self.log_joint_prob(X), axis = -1)

    @tf.function(input_signature=(tf.TensorSpec(shape=[None, None], dtype=tf.float32),))
    def compute_log_conditional_distribution(self, X):
        print("----Tracing--log_conditional_distribution")
        list_cond = tf.TensorArray(dtype =tf.float32, size =0, dynamic_size= True)
        for c in tf.range(self.y_unique_t.shape[0]):
            list_cond = list_cond.write(c, tf.reduce_logsumexp(self.compute_log_pdf(X, c), axis = -1) + tf.math.log(self.logits_y[c] + self.stable))
    ...
    def share_loss(self, X, black_box_model):
        ...
        return tfp.monte_carlo.expectation(
            f=lambda x: kl(
                ...
                black_box_model(x, trainable = tf.constant(False))
                ),
            samples = X,
    ...
    def expected_ll (self, X, y, responsibilities, weights):
        ...
        for i in tf.range(len(self.y_unique)):
            list_likelihood = list_likelihood.write( i, 
                tf.reduce_mean(
                    tf.multiply(
                        tf.multiply(weights[:,self.y_unique_t[i], tf.newaxis],
                                responsibilities[self.y_unique_t[i]]
                            ),
    ...
    def __call__(self, X, y, responsabilities, weights):
        ...
        for i in tf.range(self.y_unique_t.shape[0]):
            list_likelihood =list_likelihood.write(i, 
                tf.reduce_mean(
                    tf.multiply(
                        tf.multiply(weights[:,self.y_unique_t[i], tf.newaxis],
                                responsabilities[self.y_unique_t[i]]
                            ),
                        # tf.gather_nd(responsabilities[self.y_unique[i]],
                        #             tf.where(tf.equal(y, self.y_unique[i]))
                        #             ),
        ...
        #New losss:
        return - tf.reduce_sum(list_likelihood.stack()) - tf.reduce_sum(prior) #+ noverlap()



    @tf.function    
    def compute_responsibilities(self, X, y):
        print("-----Tracing-compute_responsibilities")
